...
class ChartManager {
  ...
  drawChartElements(chartData) {
    ...
    // 绘制趋势段
    if (barsResult.qsds) {
      barsResult.qsds.forEach((qsd) => {
        if (qsd.points?.[0]?.time >= from) {
          const key = JSON.stringify(qsd);
          const existed = chartContainer.qsds.find((item) => item.key === key);
          if (existed) return;
          chartContainer.qsds.push({
            time: qsd.points[0].time,
            key,
            id: ChartUtils.createLineShape(this.chart, qsd, {
              color: CHART_CONFIG.COLORS.QSD,
              linewidth: 3,
            }),
          });
        }
    ...
    // 绘制趋势段中枢
    if (barsResult.qsd_zss) {
      barsResult.qsd_zss.forEach((qsd_zs) => {
        if (qsd_zs.points?.[0]?.time >= from) {
          const key = JSON.stringify(qsd_zs);
          const existed = chartContainer.qsd_zss.find(
            (item) => item.key === key
          );
          if (existed) return;
          chartContainer.qsd_zss.push({
            time: qsd_zs.points[0].time,
            key,
            id: ChartUtils.createZhongshuShape(this.chart, qsd_zs, {
              color: CHART_CONFIG.COLORS.QSD_ZSS,
              linewidth: 2,
            }),
          });
        }
