...
def alert_list(market):
    ...
    task_error = services.guard_task(services.alert_tasks)
    if task_error is not None:
        return {'code': 1, 'msg': task_error['msg'], 'count': 0, 'data': []}
    al = services.alert_tasks.task_list(market)
...
def alert_edit(market, id):
    ...
    if id != '0':
        _alert_config = services.alert_tasks.alert_get(id)
...
def alert_save():
    ...
    services.alert_tasks.alert_save(alert_config)
    return {'ok': True}
...
def alert_del(id):
    ...
    if task_error is not None:
        return task_error
    res = services.alert_tasks.alert_del(id)
    return {'ok': res}
...
def xuangu_task_list(market):
    services = get_web_services()
    task_error = services.guard_task(services.xuangu_tasks)
    if task_error is not None:
        return task_error
    zx = services.zixuan_factory(market)
    zixuan_groups = zx.zixuan_list
    frequencys = services.get_exchange(Market(market)).support_frequencys()
    xuangu_task_configs = services.xuangu_tasks.xuangu_task_config_list()
...
def xuangu_task_add():
    ...
    if task_name not in services.xuangu_tasks.xuangu_task_config_list().keys():
        return {'ok': False, 'msg': '选股任务不存在'}
    allow_freq_num = services.xuangu_tasks.xuangu_task_config_list()[task_name].get('frequency_num', len(frequencys))
    if len(frequencys) != allow_freq_num:
        return {'ok': False, 'msg': f'选股周期错误，该任务可选周期数量 : {allow_freq_num}'}
    run_res = services.xuangu_tasks.run_xuangu(market, task_name, frequencys, src_zx_group, target_zx_group)
