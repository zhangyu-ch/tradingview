...
> **CTP 当前不可用。** `exchange_ctp.py` / `trader_ctp.py` 仍是未完成实现，
> CR-05 尚未修复。不要将 `EXCHANGE_FUTURES` 配置为 `"ctp"`，也不要在实盘脚本中
> 直接实例化 CTP 类。国内期货行情请使用 `tq`、`tdx_futures` 或 `db`。如果误配置为
> `ctp`，交易所工厂会明确拒绝启动，而不会加载未完成的 CTP 模块。
...
同一个市场内，监控任务名称是任务的业务标识。例如 A 股市场中只能存在一个
“价格突破监控”；港股市场仍可使用相同名称。新增或修改任务时，如果同一市场已经
存在同名任务，页面会提示修改原任务或换一个名称，不会再保存第二条重复记录。

升级旧数据库时，程序会自动检查 `cl_alert_task`：

- 没有重复任务时，只补建 `(market, task_name)` 唯一索引。
- 已存在重复任务时，不会直接删除任何一行。程序优先保留正在运行的任务；状态相同
  时保留最近修改（再按较大 ID）的任务。
- 其余重复行会被重命名为带 `[duplicate-disabled-ID]` 后缀的名称，并设置为停用，
  方便在监控任务页面核对、手工合并或删除。
- 迁移完成后由数据库唯一索引兜底，即使两个请求同时保存，也不能再次产生同市场
  同名任务。

如果升级日志出现“检测到历史重复监控任务”，请进入监控任务页面核对被停用的记录；
确认配置已经合并后再手工删除，不要直接修改数据库索引。
