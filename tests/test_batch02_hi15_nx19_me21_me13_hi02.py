...
def test_hi02_legacy_or_test_task_without_message_flag_defaults_to_no_send(monkeypatch):
    """Older task-like objects without is_send_msg must not crash after the upgrade."""
    import importlib
    import sys
    ...
    alert_tasks = importlib.import_module("cl_app.alert_tasks")
    ...
    event = SimpleNamespace(
        ...
        message="signal",
        score=1.0,
        event_time=dt.datetime(2026, 7, 31, 10, 0),
    )
    sent = []
    ...
    monkeypatch.setattr(alert_tasks, "ZiXuan", lambda market: SimpleNamespace(zx_stocks=lambda group: [{"code": event.code, "name": event.name}]))
    monkeypatch.setattr(alert_tasks, "load_registered_strategy", lambda *args, **kwargs: object())
    monkeypatch.setattr(alert_tasks, "MonitoringRunner", lambda **kwargs: SimpleNamespace(run_code=lambda *args, **kw: [event]))
    monkeypatch.setattr(alert_tasks, "db", SimpleNamespace(alert_event_save=lambda **kwargs: None))
    monkeypatch.setattr(alert_tasks.utils, "send_fs_msg", lambda *args, **kwargs: sent.append((args, kwargs)))

    assert alert_tasks.AlertTasks(None).alert_run("legacy") is True
    assert sent == []
