...
- `src/tradingview_zy/`：核心库。`cl.py` 实现缠论计算，`cl_interface.py` 定义 K 线、分型、笔、线段、中枢等接口/数据结构。
...
- `src/tradingview_zy/backtesting/`：回测框架。`backtest.py` 的 `BackTest` 组合 `BackTestKlines`、`BackTestTrader` 和策略类；`base.py` 定义 `Strategy`、`Trader`、`MarketDatas`、`POSITION`、`Operation` 等基类。
