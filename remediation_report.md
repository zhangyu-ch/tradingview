...
- **本轮状态：** 已完成
- **问题是否存在：** 是
- **a. 这个问题是什么？** TableByAlertTask 将通用 strategy_config/strategy_memo 继续映射到旧 check_idx_ma_info/check_idx_macd_info 两个 String(200) 列。SQLite 不执行 VARCHAR 长度，而 MySQL 可能报错或截断；现有 create_all 也不会为已部署表改变列结构。Web 对 JSON 与备注没有独立 UTF-8 字节边界，保存后没有往返完整性验证。
- **b. 我是怎么修复的？** 新增独立 Text 物理列 strategy_config 与 strategy_memo，ORM 通过 strategy_config_text/strategy_memo_text 映射，属性优先读新列并保留旧列只读 fallback。DB 启动在 create_all 后执行幂等迁移：为旧表添加两列并回填 legacy 数据。strategy 专用保存/更新不再写旧 String(200)，而是在单事务内写新列、flush/refresh 后逐字节核对，任何静默截断都回滚。新增统一存储协议：config 必须是标准 JSON 对象，拒绝 NaN/Inf，按 UTF-8 字节限制为 32 KiB；memo 限制 8 KiB 且拒绝 NUL。Web 在 json.loads、策略导入和 DB 调用前执行边界校验。
- **c. 修复后是否验证？** 是
- **d. 怎么验证的？**
  - 运行 `PYTHONPATH=src python -m pytest -q -W error tests/test_nx10_strategy_storage.py`，6 项专项测试通过。
  - 用 legacy SQLite 表动态执行迁移，确认新增 strategy_config/strategy_memo TEXT、旧值/NULL 回填、第二次迁移零变更。
  - 在独立子进程导入真实 DB 单例，保存并更新超过 200 字符的中英文 JSON 与长备注；查询新物理列、ORM 属性和 MySQL CreateTable DDL，全部完整。
  - 验证 JSON 非对象、破损 JSON、NaN/Inf、UTF-8 多字节超限、memo NUL/超限均在写库前拒绝。
  - 源码顺序门禁确认 raw strategy_kwargs 先检查字节再 json.loads，最终 payload 在 `_alert_tasks.alert_save` 前规范化；DB 保存后执行 refresh 与精确比较。
  - 运行 NX-10、ME-18、ME-19、ME-26、NX-21、NX-22、NX-23、strategy loader 相邻组合，53 passed、2 deselected（两项仅因归档缺失本地 config.py）。
  - 执行 py_compile、git diff --check 与 db.py/Web 历史 CRLF bare-LF=0 检查。
- **e. 验证是否通过？** 通过（6 项专项、53 项可运行相邻组合通过；2 项既有环境阻断被明确排除；迁移、长 JSON 往返、MySQL TEXT DDL、保存后完整性和请求边界均已验证）
- **提交：** fix(NX-10): migrate alert strategy storage
- **修改文件：** `src/tradingview_zy/alert_strategy_storage.py`, `src/tradingview_zy/db.py`, `web/tradingview_zy_chart/cl_app/__init__.py`, `tests/test_nx10_strategy_storage.py`, `audit/remediation_state.json`, `remediation_report.md`, `findings.md`, `progress.md`, `task_plan.md`
- **验证限制：**
  - 没有连接真实 MySQL；新表 DDL 用 SQLAlchemy MySQL dialect 编译，运行时迁移与往返在真实 SQLite 执行。
  - 旧列中已经在历史 MySQL 部署被截断的数据无法恢复；迁移只能无损复制当前仍存在的值。
  - legacy check_idx_* 列暂时保留为兼容 fallback，不再由新的 strategy 专用写路径更新；后续 schema 大版本可在确认所有部署迁移后删除。
  - 32 KiB/8 KiB 是应用硬上限，故意低于 MySQL TEXT 64 KiB；若未来需要更大配置，应版本化变更列类型和边界，而不是放宽其中一端。
  - 两项旧 DB 子进程测试在产品逻辑前被归档未含 config.py 阻断；NX-10 自己的真实 DB 子进程注入了最小临时 SQLite 配置并通过。
...
- **本轮状态：** 已完成（未实现公开费用桩删除）
- **问题是否存在：** 是
- **a. 这个问题是什么？** backtesting.base 公开定义 fee_us(opt, price, amlunt)，函数体只有 pass，调用会静默返回 None；全仓 src/script/web 又没有任何调用方或兼容需求，继续保留会让维护者误认为美股费用已经实现。
- **b. 我是怎么修复的？** 删除无实现、无调用方的 fee_us 公开函数，不伪造未经数据来源验证的美股费率；保留已实现的 A 股 fee_a 行为不变。新增 AST 与动态模块门禁，防止同名空桩或引用重新进入运行代码。
- **c. 修复后是否验证？** 是
- **d. 怎么验证的？**
  - 对 src、script、web 下全部 Python 文件执行 AST 扫描，确认 fee_us 定义和名称引用均为 0。
  - 直接加载真实 backtesting/base.py，确认模块不再暴露 fee_us。
  - 动态复算 fee_a 买入和卖出样例，结果仍分别为 32.0 与 42.0，证明相邻已实现费用函数未被破坏。
  - 运行 NX-09 与 NX-08 回测模型相邻测试，结果 6 passed（-W error）；py_compile、CRLF 与 git diff --check 通过。
- **e. 验证是否通过？** 通过（未实现函数和全部运行代码引用均已删除，既有 A 股费用计算及 6 项相邻测试通过）
- **提交：** fix(NX-09): remove unimplemented US fee stub
- **修改文件：** `src/tradingview_zy/backtesting/base.py`, `tests/test_nx09_removed_us_fee_stub.py`, `audit/remediation_state.json`, `remediation_report.md`, `findings.md`, `progress.md`, `task_plan.md`
- **验证限制：**
  - 本条按原问题建议选择删除无调用方空桩，没有实现美股费用模型；未来恢复该能力时必须使用有来源、版本和生效日期的数据驱动费率，并增加黄金样本测试。
- **原报告最新结论：** 当前 master 的相关实现路径（src/tradingview_zy/backtesting/base.py）仍保留 V6 已确认的错误模式；PR #15 未提供能够消除根因的实现或专项测试。
...
- **本轮状态：** 已完成（运行事实、支持矩阵与历史研究目录已对齐）
- **问题是否存在：** 是
- **a. 这个问题是什么？** 原报告关于 check_env 的版本漂移已由 ME-24 修复，但剩余文档漂移仍真实存在：README 和首页关于页继续宣称仓库是“交易执行工具”，与 CR-03 所有实盘 order() fail-closed 冲突；顶层 joinquant/ 仍像活跃模块一样存在，却直接依赖专有 jqdata 和已移除的 cl 包；provider-capabilities.md 只有手写说明，没有从 MarketRegistry 生成、由 CI 拒绝陈旧内容的全量支持矩阵。
- **b. 我是怎么修复的？** 保留 ME-24 已验证的 check_env 实现，不做重复改写。把原顶层 JoinQuant notebooks、README 和 helper 以固定 1980 时间戳、固定权限和排序条目确定性打包为 archive/joinquant-legacy.zip，删除活跃 joinquant/ 根目录，并新增 archive/README.md 明确历史代码不可运行。校准 README 与 Web 关于页，删除实盘交易能力误述，明确仅有市场数据、研究、监控、回测及部分只读账户能力，所有 live order 继续 fail-closed。新增 generate_provider_support_matrix.py，从 MarketRegistry 生成市场、默认 provider、默认代码、TradingView session/timezone 和每个 provider 精确 Capability 的确定性 Markdown；repository-hygiene CI 与 check_quality_gates.py 必须执行 --check，任何 Registry 变化未更新文档都会失败。
- **c. 修复后是否验证？** 是
- **d. 怎么验证的？**
  - 执行 LO-08 专项 7 项（-W error）：验证 check_env 从 pyproject 读取 >=3.11,<3.12，README 的 Python/uv/locked 安装说明一致。
  - 验证 README 和首页不再出现“交易执行工具”“trader 下单”等过期声明，均链接 live-trading-disabled 与自动支持矩阵，并说明 JoinQuant 仅在 archive 留存。
  - 打开 archive/joinquant-legacy.zip，验证恰有原四个文件、全部固定 1980 时间戳和 100644 权限；fun.py 中 jqdata/cl 依赖仅存在于归档，活跃 joinquant/ 不存在。
  - 将 docs/provider-support-matrix.md 与 render_support_matrix(MARKET_REGISTRY) 做字节级精确比较；遍历全部市场/provider/capability，确认矩阵完整且没有内置 provider 声明 LIVE_ORDERS。
  - 故障注入在临时矩阵追加 manual drift，执行生成器 --check 返回非零；从临时 repository-hygiene workflow 删除矩阵检查命令，check_quality_gates.py 稳定报告缺失。
  - 运行 LO-08、ME-24、ME-29 quality gates、CR-03、LO-05、LO-07 严格组合：50 passed（-W error）。
  - 仓库级可运行回归：626 passed、5 skipped、8 deselected；8 个节点仍因当前容器缺少 pinyin 在 cl_app 导入阶段阻断，完整套件另受缺少 empyrical 的回测收集阻断。
  - 执行 compileall、support-matrix --check、quality/repository/readability/dependency、Secret、supply-chain、FIFO、JSON、git diff 与 README/index CRLF 门禁，全部通过。
- **e. 验证是否通过？** 通过（7 项文档专项、50 项严格相邻及 626 项可运行仓库回归通过；文档漂移已变成 CI 可执行失败）
- **提交：** docs(LO-08): generate support truth and archive legacy research
- **修改文件：** `README.md`, `web/tradingview_zy_chart/cl_app/templates/index.html`, `archive/README.md`, `archive/joinquant-legacy.zip`, `joinquant/#U6570#U636e#U4e0b#U8f7d.ipynb`, `joinquant/A#U80a1#U52a8#U91cf#U6392#U884c#U9009#U80a1#U62e9#U65f6.ipynb`, `joinquant/README.md`, `joinquant/fun.py`, `docs/provider-support-matrix.md`, `docs/provider-capabilities.md`, `docs/quality-gates.md`, `script/remediation/generate_provider_support_matrix.py`, `script/remediation/check_quality_gates.py`, `.github/workflows/repository-hygiene.yml`, `tests/test_lo08_documentation_contract.py`, `audit/remediation_state.json`, `remediation_report.md`, `findings.md`, `progress.md`, `task_plan.md`
- **验证限制：**
  - 生成矩阵证明标准 factory 的声明能力和文档一致，不替代真实券商、交易所或数据供应商 sandbox/生产验收；该限制在矩阵与质量门禁文档中明确保留。
  - JoinQuant 遗留仅作为历史研究证据归档，未审查其专有平台许可、未恢复依赖，也不承诺可执行；恢复必须新建能力、依赖/许可评审和测试。
  - README 仍包含手写操作说明，但关键 Python、锁文件、实盘禁用、市场/provider 能力和历史目录边界均已有自动测试或生成门禁。
  - 当前容器缺少 pinyin 与 empyrical；环境阻断已单独记录，CI 的 Python 3.11 锁定环境仍要求完整 pytest。
...
- **本轮状态：** 已完成（市场、周期与订单代码统一领域化）
- **问题是否存在：** 是
- **a. 这个问题是什么？** 实际重建仓库中 Market 仍为普通 Enum，Frequency、OrderSide、PositionSide、OrderOffset、OrderStatus 等报告曾提及的领域类型并不存在；provider、策略、回测和数据库公共边界继续接受任意 market、frequency、operation 字符串。未知拼写与控制字符可能进入 SDK 或 SQL，Operation 依赖 open/close 到 buy/sell 的临时字典，provider 周期 metadata 也没有穷尽校验。问题真实存在。
- **b. 我是怎么修复的？** 把 Market 改为可稳定 JSON/SQL 序列化的 StrEnum，并新增 Frequency、OrderSide、PositionSide、OrderOffset、OrderStatus、OperationAction、TradeMode 及严格 parser，覆盖当前全部 provider 周期代码和必要旧别名。MarketRegistry 启动时穷尽验证所有展示/同步周期；get_exchange 与 ContractedExchange 在 provider 构造或 SDK 调用前规范化市场和周期，并拒绝未声明周期。策略 target/context/signal、Web UDF resolution、回测 Operation/MarketDatas/Trader/费用入口以及 DB K 线表路由、查询、写入与删除统一在边界解析为领域代码，向 JSON、SDK 与 SQL 只输出稳定 `.value`；空值、非字符串、控制字符、未知拼写和未来未登记周期全部 fail-closed。
- **c. 修复后是否验证？** 是
- **d. 怎么验证的？**
  - 新增 LO-03 专项 11 项并以 warnings-as-errors 执行：验证 Market/Frequency 稳定字符串与 JSON 序列化、所有 provider support_frequencys AST 字面量均可解析、控制字符/未知值/非字符串拒绝，以及订单方向、仓位、开平、状态、操作和模式的允许别名。
  - 使用 fake raw provider 验证非法或未支持 frequency 在 raw SDK 的 klines() 前失败；构造重复或非法 metadata 时 ContractedExchange 在 facade 发布前拒绝。
  - 验证 strategy_target_from_stock、StrategySignal canonicalization/to_payload、未知市场 target 阶段拒绝、Web resolution 解析与 Registry 周期穷尽校验；策略和 provider 不会在非法代码上执行。
  - 隔离加载真实 backtesting/base.py，验证 Operation、MarketDatas、Trader 和 fee_a 的兼容序列化及非法操作/市场/模式前置拒绝；完整历史回测收集仍由当前环境缺少 empyrical 阻断。
  - 使用真实 SQLite DB 执行 K 线插入和查询，确认市场与周期按稳定 value 落库；非法市场/周期在建表、查询或写入前失败且不产生记录。
  - 运行 LO-03 与 LO-02、LO-05、LO-07、ME-10、ME-18、ME-20、MX-18、NX-17、NX-23、RV-07 严格组合：193 passed（-W error）。
  - 临时准备受控 config 后执行仓库级可运行回归：637 passed、5 skipped、8 deselected；8 个节点均因当前容器缺少 pinyin 在 cl_app 导入阶段阻断，完整套件另有缺少 empyrical 的回测收集阻断。
  - 执行 compileall、支持矩阵、quality/repository/readability/dependency、Secret、supply-chain、FIFO、JSON、git diff 与全部变更 CRLF 门禁，全部通过。
- **e. 验证是否通过？** 通过（11 项专项、193 项严格相邻、637 项可运行仓库回归及全部静态门禁通过）
- **提交：** refactor(LO-03): enforce typed market and order codes
- **修改文件：** `src/tradingview_zy/base.py`, `src/tradingview_zy/domain.py`, `src/tradingview_zy/market_registry.py`, `src/tradingview_zy/exchange/__init__.py`, `src/tradingview_zy/exchange/contracted.py`, `src/tradingview_zy/strategies/base.py`, `src/tradingview_zy/backtesting/base.py`, `src/tradingview_zy/db.py`, `src/tradingview_zy/web_api_validation.py`, `tests/test_lo03_domain_codes.py`, `tests/test_me18_strategy_runner_contracts.py`, `audit/remediation_state.json`, `remediation_report.md`, `findings.md`, `progress.md`, `task_plan.md`
- **验证限制：**
  - 本条只统一公共输入、领域协议、SDK/JSON/SQL 序列化边界；第三方 SDK 内部 payload 和历史 DataFrame/dict 结构将在第 79 条 LO-04 的 Data Clumps 边界继续治理。
  - PositionSide 与 OrderStatus 已建立严格领域代码，但所有实盘下单仍由 CR-03 无条件 fail-closed；未执行券商状态映射或真实交易联调。
  - 当前容器缺少 pinyin 与 empyrical；相关节点在进入本条产品断言前阻断，托管 Python 3.11 锁定环境仍要求完整 pytest。
