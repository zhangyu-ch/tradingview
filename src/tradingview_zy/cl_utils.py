...
from tradingview_zy.cl_interface import BI, FX, ICL, LINE, MACD_INFOS, ZS, Config, Kline
from tradingview_zy.db import db
from tradingview_zy.exchange import exchange
...
def cl_data_to_tv_chart(
    ...
    qsd_chart_data = []
    if config["chart_show_qsd"] == "1":
        for qsd in cd.get_qsds():
            qsd_chart_data.append(
                {
                    "points": [
                        {
                            "time": fun.datetime_to_int(qsd.start.k.date),
                            "price": qsd.start.val,
                        },
                        {
                            "time": fun.datetime_to_int(qsd.end.k.date),
                            "price": qsd.end.val,
                        },
                    ],
                    "linestyle": "0" if qsd.is_done() else "1",
                }
            )
    ...
    qsd_zs_chart_data = []
    if config["chart_show_qsd_zs"] == "1":
        for zs in cd.get_qsd_zss():
            qsd_zs_chart_data.append(
                {
                    "points": [
                        {"time": fun.datetime_to_int(zs.start.k.date), "price": zs.zg},
                        {"time": fun.datetime_to_int(zs.end.k.date), "price": zs.zd},
    ...
    qsd_chart_data.sort(key=lambda v: v["points"][0]["time"], reverse=False)
    ...
    qsd_zs_chart_data.sort(key=lambda v: v["points"][0]["time"], reverse=False)
